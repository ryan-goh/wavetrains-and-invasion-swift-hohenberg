function branch = SecantContinuation(problem,u0,p0,stepperPars)
   ...
   while step < maxSteps && ~continuationFailed && ~reachedBound
	 ...
	 reachedBound = true;
       ...
       %JR: check user defined stopping condition
       if uzstop(v1,v0); reachedBound=true; end
	 ...
	 numUnstableEigenvalues = numel( find( real(d) > 0 ) );
       ...
       % Output
       if (mod(step,nPrint) == 0) || (reachedBound)
	 ...
	 if ~isempty(PlotSpectrum)
       ...
       if (mod(step,nSaveSol) == 0) || (reachedBound)
   ...
   function plotHandle = PlotBranch(branch,idVar,parentHandle)
     ...
     figure(parentHandle)
     cla, hold on;
     plot(real(branch(:,3)),real(branch(:,idVar)),'b-')
     plot(real(branch(end,3)),real(branch(end,idVar)),'rd','MarkerSize',10);
     if ~isempty(ComputeEigenvalues)
       ...
       plot(real(branch(:,3)),real(branch(:,idVar)),'k.')
     end
     drawnow;
