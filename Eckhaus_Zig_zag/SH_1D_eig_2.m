% solve the Eckhaus boundary value problem for a given mu find a k for the EK
% boundary via setting lam_σσ = 0
function [F,J] = SH_1D_eig_2(uin,p,mesh_params)
  ...
  lamg = uin(4*n+3);
  k    = uin(4*n+4);
  lamgg = 0; % impose the EK stability boundary condition
  ...
  F4 = L*wgg + 2*lamg*wg + 8*(speye(n) + k^2*D2z)*k^2*Dz*wg + 4*k^2*w + 12*k^4*D2z*w;
